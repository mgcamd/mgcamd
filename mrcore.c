...
/* Ensure that the array has correct alignment */
    ...
    x=(big)mr_alloc(_MIPP_ sizeof(mr_unsign32)+sizeof(mr_small *)+(mr_mip->nib+1)*sizeof(mr_small),1);
    if (x==NULL)
    {
        MR_OUT 
        return x;
...
/*** Multi-Threaded support ***/
    ...
    mr_mip->w16=mirvar(_MIPP_ 0); 
    mr_mip->w17=mirvar(_MIPP_ 0); 
    mr_mip->w18=mirvar(_MIPP_ 0); 
    mr_mip->depth--;
    return mr_mip;
} 
...
{ /* clean up after miracl */
    ...
    mirkill(_MIPP_ mr_mip->w16);
    mirkill(_MIPP_ mr_mip->w17);
    mirkill(_MIPP_ mr_mip->w18);
#ifdef MR_FLASH
